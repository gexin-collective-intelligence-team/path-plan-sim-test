        ...
        for row in range(self.rows):
            for col in range(self.cols):
                pygame.draw.rect(self.screen, (0, 0, 0),
                                 ((col * self.cell_size) + self.padding, (row * self.cell_size) + self.padding, self.cell_size, self.cell_size), 1)

    def draw_points(self):
        start_rect = pygame.Rect(self.start_point[1] * self.cell_size + self.padding, self.start_point[0] * self.cell_size + self.padding, self.cell_size, self.cell_size)
        end_rect = pygame.Rect(self.end_point[1] * self.cell_size + self.padding, self.end_point[0] * self.cell_size + self.padding, self.cell_size, self.cell_size)
